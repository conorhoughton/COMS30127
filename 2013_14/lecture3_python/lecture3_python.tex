...
These are getting started notes on Python. I offer an error bounty of
...
or last and complicated networks of neurons, in the case of NEST. The
...
>>> 
\end{lstlisting}
where the numbers on the right are just for these notes.
...
\texttt{math.h} in C or \texttt{cmath} in C++. Notice the namespacing, you need to write \texttt{math.tan(math.pi/4.0)}. If you want to import the functions and so on into your namespace you use \texttt{from}, eg
\begin{lstlisting}[numbers=right]
...
not the colon after the \texttt{for} statement too. This sort of for
...
print list
\end{lstlisting}
Python also has a tuple type, like a list but immutable. 
...
them in the context of a very annoy / fun puzzle try
\texttt{http://www.pythonchallenge.com/}. Here we will quickly
introduce the packages commonly used in scientific computing 
\begin{itemize}
