...
proved useful for machine learning. It has been proposed,
...
randomize the connectivity between EC and CA3. DG is a feedforward
network, this means that there is few lateral connections between the
principle excitatory cells so the message just gets passed
forward. This makes the role of the DG not so clear and the idea that
has been proposed is that going through DG randomizes the patterns and this seperates them for storage. In
short, during learning neurons in EC and CA3 are matched via DG and
that the connections from EC to DG and from DG to EC are essentially
random. This reduces overlap in the patterns being stored in DG.
